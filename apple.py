...
for i in range(6, n+1):
    ultimi_6 = cambiamenti[i-6:i]
    salite = sum(1 for c in ultimi_6 if c == 0.02)
    y = 1 if salite >= 3 else -1
    ys.append(y)
...
plt.subplot(2, 1, 2)
plt.plot(range(6, n+1), ys, 'xr')
plt.title('Condizione soddisfatta (1) o no (-1) per ogni step')
plt.xlabel('Step (a partire dal giorno 6)')
plt.ylabel('y')
plt.yticks([-1, 1])

plt.tight_layout()
plt.savefig('fig/simulazioni_azioni.png')
plt.show()
